...
execute if score .server elements_stage_3_boss_status matches 1 unless entity @e[tag=stage-3_boss] if score .server elements_custom-items_shop_boss-frucht_remain matches 1 in elements:stage-3_bossroom run setblock 0 101 -1 chest[facing=east]{LootTable:"elements:stage-3_boss_key"} replace
