...
execute if score .server elements_stage_3_boss_melee_timer matches 0 as @a at @s if entity @e[tag=stage-3_boss,distance=..10] run damage @s 25 elements:boss_stage_3
...
execute if score .server elements_stage_3_boss_status matches 1 unless entity @e[tag=stage-3_boss] in elements:stage-3_bossroom run give @a[distance=0..,nbt={Inventory:[{id:"minecraft:mace"}]},gamemode=!spectator] breeze_rod 2
...
execute in elements:stage-3_bossroom if entity @e[tag=stage-3_boss] run bossbar set elements:stage-3_boss players @a[distance=0..]
