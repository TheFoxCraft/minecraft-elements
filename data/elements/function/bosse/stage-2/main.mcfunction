...
execute if score #server elements_stage_2_boss_status matches 1 unless entity @e[tag=stage-2_boss] if score #server elements_custom-items_shop_boss-frucht_remain matches 0 in elements:stage-2_bossroom run setblock 0 101 -1 chest[facing=east]{Items:[{Slot:13b,id:"minecraft:stripped_bamboo_block",count:1,components:{"minecraft:item_name":'{"color":"yellow","text":"Holz-Boss Schlüssel"}',"minecraft:lore":['" "','{"color":"gray","italic":false,"text":"Öffnet den Vault im HUB für deinen Loot"}','" "','{"bold":true,"color":"blue","italic":false,"text":"Rare"}'],"minecraft:custom_model_data":1,"minecraft:enchantment_glint_override":true,damage_resistant:{types:"#is_fire"}}}]} replace
