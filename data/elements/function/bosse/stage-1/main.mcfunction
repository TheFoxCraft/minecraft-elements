...
execute as @e[type=#impact_projectiles] at @s if entity @e[tag=stage-1_boss,distance=..6.5] run particle explosion
execute as @e[type=#impact_projectiles] at @s if entity @e[tag=stage-1_boss,distance=..6.5] run kill @s
...
execute if score #server elements_stage_1_boss_status matches 1 unless entity @e[tag=stage-1_boss] in elements:stage-1_bossroom run tellraw @a[distance=0..] [{"text": "Server: ","color": "yellow"},{"text": "Der Rießige Silberfisch wurde besiegt!","color": "white"}]
...
execute if score #server elements_stage_1_boss_status matches 1 unless entity @e[tag=stage-1_boss] if score #server elements_custom-items_shop_boss-frucht_remain matches 0 in elements:stage-1_bossroom run setblock 0 101 -1 chest[facing=east]{Items:[{Slot:13b,id:"minecraft:smooth_stone",count:1,components:{"minecraft:item_name":'{"color":"yellow","text":"Stone-Boss Schlüssel"}',"minecraft:lore":['" "','{"color":"gray","italic":false,"text":"Öffnet den Vault im HUB für deinen Loot"}','" "','{"bold":true,"color":"blue","italic":false,"text":"Rare"}'],"minecraft:fire_resistant":{},"minecraft:custom_model_data":1,"minecraft:enchantment_glint_override":true}}]} replace
...
execute in elements:stage-1_bossroom store result bossbar elements:stage-1_boss max run attribute @n[tag=stage-1_boss] generic.max_health get 1
