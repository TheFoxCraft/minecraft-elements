...
#on place
execute as @a[scores={elements_farm-slots_stage-2_placed=1..},sort=nearest,limit=1] at @s run function elements_retina:traverse/setup
...
execute as @e[type=marker,tag=elements_farm-slot_stage-2] at @s if block ~ ~ ~ air run function elements:farm_slots/stage_2/break with entity @n[distance=..10,scores={elements_farm-slots_stage-2_broken=1..}]
