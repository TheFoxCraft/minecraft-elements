execute unless score @s elements_homedimension = @s elements_dimension run return run scoreboard players remove @s elements_miningxp_mining_mine-get_cocoa 1
...
execute at @s if score @s elements_miningxp_mining_level >= @s elements_miningxp_mining_loot at @n[type=item,nbt={Item:{id:"minecraft:cocoa_beans"}}] run loot spawn ~ ~ ~ loot elements:items/other/stage_6/cocoa_beans
...
execute as @s[scores={elements_custom-items_shop_mining-frucht_counter=3..}] run scoreboard players add @s elements_miningxp_mining_xp 5
