...
execute if score .server elements_spawn_levelbaum_enable matches 1 if score .server elements_spawn_levelbaum_timer_min matches 0 store result score .server elements_spawn_levelbaum_timer_min run random value 30..45
...
execute as @a[tag=spawn_levelbaum_level_player] run tellraw @a ["",{"text": "Server: ","color": "yellow"},{"selector": "@s"},{"text": " hat einen Allay gefunden!"}]
