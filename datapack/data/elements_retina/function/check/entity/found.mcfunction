...
tag @e[type=!#elements_retina:intangible] remove retina.possible_target
scoreboard players set __if_else__ retina.__variable__ 0
execute unless score $expand_entity_hitboxes retina.__variable__ = $expand_entity_hitboxes retina.__variable__ run function elements_retina:__private__/if_else/4
execute if score __if_else__ retina.__variable__ matches 0 align xyz positioned ~-1 ~-1 ~-1 as @e[type=!#elements_retina:intangible,tag=!retina.ignore,tag=!retina.executing,dx=2,dy=2,dz=2] at @s run function elements_retina:check/entity/get_hitbox
