...
execute as @e[type=item,nbt={Item:{id:"minecraft:crimson_fungus",components:{"minecraft:custom_model_data":1}},OnGround:true},predicate=elements:is_in_hub,scores={elements_stage_3_portal_enable_fungus_timer=200}] at @s unless entity @e[type=hoglin,tag=elemnets_stage_3_portal_enable_hoglin] if block ~ ~-1 ~ crimson_nylium run particle smoke ~ ~ ~ 2 2 2 0 100 force
execute as @e[type=item,nbt={Item:{id:"minecraft:crimson_fungus",components:{"minecraft:custom_model_data":1}},OnGround:true},predicate=elements:is_in_hub,scores={elements_stage_3_portal_enable_fungus_timer=200}] at @s unless entity @e[type=hoglin,tag=elemnets_stage_3_portal_enable_hoglin] if block ~ ~-1 ~ crimson_nylium run summon axolotl ~ ~ ~ {Invulnerable:1b,PersistenceRequired:1b,Tags:["elemnets_stage_3_portal_enable_hoglin"],Passengers:[{id:"minecraft:hoglin",PersistenceRequired:1b,NoAI:1b,IsImmuneToZombification:1b,CannotBeHunted:1b,Tags:["elemnets_stage_3_portal_enable_hoglin"],DeathLootTable:empty}],active_effects:[{id:"minecraft:invisibility",amplifier:255,duration:-1,show_particles:0b,show_icon:0b,ambient:0b}],attributes:[{id:"minecraft:scale",base:0.0625}]}
execute as @e[type=item,nbt={Item:{id:"minecraft:crimson_fungus",components:{"minecraft:custom_model_data":1}},OnGround:true},predicate=elements:is_in_hub,scores={elements_stage_3_portal_enable_fungus_timer=200}] at @s if entity @e[type=hoglin,tag=elemnets_stage_3_portal_enable_hoglin,distance=0] if block ~ ~-1 ~ crimson_nylium run kill @s
