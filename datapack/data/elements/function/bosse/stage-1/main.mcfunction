...
execute if score .server elements_stage_1_boss_melee_timer matches 0 as @a at @s if entity @e[tag=stage-1_boss,distance=..6] run damage @s 4 elements:boss_stage_1
...
execute if score .server elements_stage_1_boss_status matches 1 unless entity @e[tag=stage-1_boss] in elements:stage-1_bossroom run give @a[distance=0..,nbt={Inventory:[{id:"minecraft:mace"}]},gamemode=!spectator] wind_charge 2
