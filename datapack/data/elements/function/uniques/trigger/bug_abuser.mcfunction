...
function elements:sonstiges/savely_give_loot {id:"elements:items/uniques/collections/bug_abuser"}
xp add @s 25000 levels
execute at @s run playsound ui.toast.challenge_complete master @s

scoreboard players set @s elements_uniqueitem_collections_bug_abuser 1
scoreboard players set .server elements_uniqueitem_collections_bug_abuser 1
...
execute in elements:hub as @n[type=marker,tag=current_unique_item] at @s run summon item_display ~ ~ ~ {billboard:"center",Tags:["elements_unique","elements_unique_bug_abuser","elements_unique_bug_abuser_item"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[0.8f,0.8f,0.8f]},item:{id:"minecraft:barrier",count:1}}
execute in elements:hub as @n[type=marker,tag=current_unique_item] at @s run summon text_display ~ ~0.65 ~ {billboard:"center",Tags:["elements_unique","elements_unique_bug_abuser","elements_unique_bug_abuser_name"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[1f,1f,1f]},text:'{"bold":true,"color":"gold","italic":false,"translate": "elements.item.unique.bug_abuser.name"}',alignment:"center"}
...
data modify storage elements_unique_item_set_name item_id set value "bug_abuser"
...
execute in elements:hub as @n[type=armor_stand,tag=elements_unique_set] at @s run loot replace entity @s weapon.mainhand loot elements:items/uniques/collections/bug_abuser
execute in elements:hub as @n[type=armor_stand,tag=elements_unique_set] at @s run data modify entity @n[type=item_display,tag=elements_unique_bug_abuser] item set from entity @s HandItems[0]
